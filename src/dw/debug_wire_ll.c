...
uint8_t dw_ll_flash_write_populate_buffer(const uint16_t *buffer, uint16_t len, uint16_t remaining) {
    ...
    while (len--) {
        ...
        od_uart_tx_byte(*buffer & 0xff);
        dw_ll_exec(BE(AVR_INSTR_IN(r1, debug_wire_g.device.reg_dwdr)), 0);
        od_uart_tx_byte(*buffer >> 8);
...
static inline uint8_t is_bp_addr_gt(dw_sw_brkpt_t *a, dw_sw_brkpt_t *b) {
    ...
    if (!a->active && !a->stored && (b->active || b->stored))
        return 1; //se a è rimosso e b esiste -> a è sempre maggiore
...
static void dw_ll_internal_update_bp_references(void) {
    if (debug_wire_g.swbrkpt_n < 2) {
        if (!debug_wire_g.swbrkpt[0].active && !debug_wire_g.swbrkpt[0].stored) debug_wire_g.swbrkpt_n = 0; //if already 0 swbrkpt_n remains unchanged even if the condition is true.
        return;
    }
    ...
    while (
            debug_wire_g.swbrkpt_n > 0
            && !debug_wire_g.swbrkpt[debug_wire_g.swbrkpt_n - 1].active
            && !debug_wire_g.swbrkpt[debug_wire_g.swbrkpt_n - 1].stored
            )
        debug_wire_g.swbrkpt_n--;
}
...
static uint8_t
dw_ll_internal_write_breakpoints(uint16_t page_address, dw_sw_brkpt_t *bps, uint16_t bps_size, uint16_t *buffer,
                                 uint16_t buf_size) {
    ...
    while (remaining_words) {
        ...
        dw_ll_flash_read(byte_address + read, words_to_read * 2,
                         (uint8_t *) buffer);//128-64 //read this part of the word
        dw_ll_registers_write(r30, r31, &z);

        uint16_t cur_buffer_wrt = 0;
        while (cur_bp_offset < words_to_read) {
            executed++;
            remaining_words = dw_ll_flash_write_populate_buffer(buffer + cur_buffer_wrt, cur_bp_offset - cur_buffer_wrt,
                                                                remaining_words);
        ...
        remaining_words = dw_ll_flash_write_populate_buffer(buffer + cur_buffer_wrt, words_to_read - cur_buffer_wrt,
                                                            remaining_words);
...
void dw_ll_flush_breakpoints(uint16_t *buffer, uint16_t len) {
    ...
    while (bp_len < debug_wire_g.swbrkpt_n) {
        uint16_t page_address = ((((debug_wire_g.swbrkpt + bp_len)->address) / debug_wire_g.device.flash_page_end) *
                                 debug_wire_g.device.flash_page_end);
        bp_len += dw_ll_internal_write_breakpoints(page_address, debug_wire_g.swbrkpt + bp_len,
                                                   debug_wire_g.swbrkpt_n - bp_len, buffer, len);
    }
}